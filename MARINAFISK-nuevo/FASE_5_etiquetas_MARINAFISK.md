...
- ~~**Envío de la muestra en PDF por WhatsApp/email a Scanfisk Celeiro**~~ (`enviarMuestraScanfiskPorWhatsapp/Email`) — al construirlo (2026-09-20, ver `VERIFICACION_ENVIO_SCANFISK_2026-09-20.md`) se encontró que estas dos funciones dependen por completo de `estado.muestraReparto`, que solo lo rellena la importación de Excel "CARGA [super]" señalada justo arriba como **código muerto** (sin botón que la dispare) — es decir, en el HTML actual estas dos funciones tampoco son alcanzables nunca desde la interfaz real. En su lugar se construyó el envío del **PDF completo de un reparto real ya grabado** (`enviarPdfCompletoDeReparto(uid, canal)` del HTML actual, ese sí con botón real en el historial de repartos) — mismo documento (ficha + una etiqueta de muestra por producto/lote), pero sobre un dato de verdad en vez de uno que nunca llega a generarse. El contacto de Scanfisk Celeiro (`cambiarContactoScanfiskCeleiro()`) pasa de `localStorage` por ordenador a la tabla `configuracion` compartida, igual que ya se hizo con los días de caducidad.
...
**Sigue diferido:**
- **Importación de hojas Excel "CARGA [super]"** (`importarCargaYGenerarEtiquetasScanfisk`, ver arriba) — sigue siendo la única pieza real que falta; nada de lo construido después dependía de ella.
