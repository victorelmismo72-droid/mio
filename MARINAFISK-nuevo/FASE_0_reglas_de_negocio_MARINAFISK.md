...
## 13. Correcciones de `CORRECCIONES_02-09-2026_para_Code_7.md` (13/09/2026 y 25/09/2026)

Trece puntos aplicados directamente al HTML actual, fuera del proyecto de migración (los puntos 1-11 llegaron el 13/09/2026 en `..._6.md`; los puntos 12 y 13 se añadieron el 25/09/2026 en `..._7.md`, que sustituye a `_6` — mismo contenido en 1-11, ambos ficheros se conservan en el repo como histórico). Ya resueltos en el backend/frontend nuevo (nada que hacer, solo constancia) / pendientes para cuando se construya la pantalla correspondiente:
...
- **Punto 12 (nuevo, 25/09/2026 — imprimir albarán sin precios y Hoja Transfrío desde la propia fila del listado, como acciones independientes)** — pendiente, Fase 4 (interfaz/impresión). Se probó a combinar ambos documentos en un único PDF/botón y se revirtió: se imprimen en impresoras físicas distintas (papel normal vs. papel pre-impreso del transportista) y no da tiempo a cambiar de impresora en mitad de un mismo trabajo. Regla de diseño: cuando dos documentos de un mismo registro se imprimen habitualmente en impresoras distintas, deben ofrecerse siempre como acciones independientes (botones separados), nunca combinadas — ni en el listado ni al abrir el registro.
- **Punto 13 (nuevo, 25/09/2026 — autosuma con "+"/"-" en cualquier campo de peso/cantidad, tipo Excel)** — [x] ya implementado en Compras (campo Kilos, `evaluarExpresionKilos`, ver `03_FRONTEND_MARINAFISK.md` punto 2) antes incluso de recibir este documento; **confirma que el mismo patrón debe generalizarse** al campo de peso de Pedidos y a cualquier campo similar (Traspasos, Repartos) cuando se construyan. Solo números, espacios, puntos/comas decimales y +/- — nunca evaluar como código genérico (ya así en `evaluarExpresionKilos`).
