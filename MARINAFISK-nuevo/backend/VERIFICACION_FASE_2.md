...
- **Exportación a PDF** (30/08/2026, a petición de Víctor): cualquier listado se puede exportar a PDF además de CSV/JSON, con el mismo formato exacto que se ve en pantalla (misma cabecera, mismas columnas alineadas a la derecha solo cuando son de verdad una cantidad, misma fila TOTAL resaltada donde la hay) — usa `jsPDF` + `jspdf-autotable` en el navegador, servidos localmente desde `public/vendor/` (sin depender de un CDN externo). Se elige apaisado u vertical automáticamente según el número de columnas. Probado con "Pedidos (detalle para contabilidad)" contra los pedidos reales del 25/08/2026: extraído el texto del PDF y confirmado carácter a carácter — mismas 18 filas, mismos importes, misma fila TOTAL final. Probado también con un listado sin fila de totales (clientes sin actividad) para confirmar que no rompe nada.
- Sigue pendiente: los "modelos de impresión" del punto 5 de la Fase 1 (fichas de envío, etiquetas, Transfrío) — **estos ya se construyeron en la Fase 4** (ver `VERIFICACION_FASE_4.md`), este punto se refería solo a los listados de gestión de este fichero.
