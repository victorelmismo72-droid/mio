...
Both funcionan igual: se valida el Excel entero primero (código y nombre son obligatorios, código repetido en el propio Excel es un error), y solo si no hay ningún error se aplica todo de golpe. Los que ya existían y cambian algún campo se actualizan; los nuevos se dan de alta; los que ya existían y NO aparecen en el Excel **se conservan tal cual** — importar un Excel parcial nunca borra lo que no menciona. `formato_etiqueta` (solo en clientes) sigue sin tocarse por Excel: no se ha pedido.

**Añadido el 20/09/2026, a petición de Víctor:** una columna opcional "TIPO IVA" en las hojas CLIENTES y PROVEEDORES — el Excel original de Víctor no la traía (por eso los 51 proveedores reales entraron migrados como `NACIONAL`, ver `VERIFICACION_IVA_PROVEEDORES_2026-09-20.md`), así que es una columna **nueva**, no una réplica de algo que ya existiera. Si una fila no trae esa columna (o la celda está vacía), el tipo de IVA se conserva igual que antes — no se pisa por omisión. Si la trae, admite tanto el valor exacto de la base de datos como algún sinónimo razonable en español ("Recargo de Equivalencia", "UE"…); un valor no reconocible bloquea la importación entera con un error explícito por fila, igual que un código repetido — un tipo de IVA mal escrito tiene consecuencias fiscales reales, así que nunca se adivina ni se ignora en silencio.
